            ...
            <div class="">
                ...
                @foreach (getMenuLink($title->slug) as $link)
                    <a href="{{ route($link->url) }}"
                        class="flex gap-2 items-center p-3 rounded-md {{ request()->is("admin/$link->name") || request()->is("admin/$link->name/*") ? 'text-white bg-gradient-to-r from-teal-600 to-teal-700' : '' }}">
                        <i class="{{ $link->icon }}"></i>
